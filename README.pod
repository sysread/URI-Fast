...
the I<encoded> value. They return the decoded value. Any syntactic characters
in the field will remain encoded (for example, encoded slashes embedded in a
path segment will remain encoded). There are some caveats to this. See
L</ENCODING> for an in depth description of their behavior.
